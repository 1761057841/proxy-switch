...
- 控制台「代理模式」区新增当前节点行：显示当前模式对应组（PROXY/自动选择/故障转移）的实际连接节点
- 节点右侧延迟徽章（毫秒）：页面加载/模式切换时自动组测速一次，结果缓存复用
- 3 秒定时跟随节点变化（auto 模式下节点切换实时可见）；手动 ⚡ 测速结果同步显示
- 延迟颜色：🟢 绿 <200ms / 🟡 黄 200-500ms / 🔴 红 >500ms

**修复：实时速度/总量跳变与超长数值**

- 根因：总量之前从 `/connections` 汇总（活动连接会话累计），连接断开会骤降归零，导致速率差分算出巨大数值
- 修复：总量改用 mihomo `/traffic` 的 upTotal/downTotal（socket 直连读 2 条）——进程级累计单调递增，连接断开不影响
- 速率 = 两次快照差值 ÷ 时间（基于稳定进程级总量）
- 前端 fmtBytes B 分支取整（0.333 B/s → 0 B/s），不再显示超长小数
